...
export async function generateMetadata({ params }: VineyardsPageProps): Promise<Metadata> {
  ...
  return (
    <>
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.4)), url('/images/vineyard.jpg') center/cover no-repeat",
        }}
      ...
      <section className="section" aria-labelledby="vineyards-heading">
        <div className="container mx-auto px-6 max-w-[1200px]">
          <h2 id="vineyards-heading" className="sr-only">{t("title")}</h2>
                ...
                >
                  ...
                  <div>
                    ...
                    <dl className="grid grid-cols-2 gap-4">
                      {[
                        { label: t("elevation"), value: `${vineyard.elevationM}m` },
                        { label: t("established"), value: String(vineyard.established) },
                        { label: t("soil"), value: t(`${key}.soil`) },
                        { label: t("varieties"), value: vineyard.varieties.join(", ") },
                        { label: t("rootstock"), value: vineyard.rootstock },
                      ].map(({ label, value }) => (
                        <div key={label} className="border-l-2 border-[var(--color-gold-medium)] pl-4">
                          <dt className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] font-light mb-1">
                            {label}
                          </dt>
                          <dd className="text-sm font-light text-[var(--color-dark)]">{value}</dd>
                        </div>
                      ))}
                    </dl>
                  </div>
      ...
      <section className="section-alt" aria-labelledby="organic-heading">
        <div className="container mx-auto px-6 max-w-[1200px]">
          <div className="max-w-3xl mx-auto text-center fade-in">
            ...
            <h2 id="organic-heading" className="section-title">{t("organic.title")}</h2>
            ...
            <div className="inline-flex items-center gap-3 border border-[var(--color-gold)] rounded-full px-8 py-3">
              <span className="text-[var(--color-gold)] text-lg" aria-hidden="true">✦</span>
